...
class _LandscapePlayerDesktopState extends State<LandscapePlayerDesktop> {
  ...
  void setSource() {
    if (widget.isNetwork) {
      ...
      setState(() {
        source = Media.file(File(widget.RawDataSource), startTime: seekto());
      });
    }
  }
  ...
  Future<String> getData_Video_web() async {
    var response = await http.get(
        Uri.parse("https://www.animeworld.tv/api/episode/info?alt=0&id=" +
            widget.RawDataSource),
        headers: {
          "x-requested-with": "XMLHttpRequest",
          "user-agent":
              "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36"
        }..addAll(globals.AWCookieTest));
    String link = json
        .decode(response.body)['grabber']
        .replaceAll("http", "https")
        .replaceAll("httpss", "https");
    setState(() {
      source = Media.network(link, startTime: seekto());
    });
  }
